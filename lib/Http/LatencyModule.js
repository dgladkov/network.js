...
export default class LatencyModule extends HttpModule {
    ...
    {
        if (typeof supportsResourceTiming !== 'boolean') {
            supportsResourceTiming = Timing.supportsResourceTiming;
        }
        ...
        this.off(['xhr-load', 'xhr-loadstart', 'xhr-readystatechange', 'xhr-timeout']);

        this.on('xhr-timeout', () => {
          console.log('Timeout while measuring latency');
