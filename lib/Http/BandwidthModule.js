...
export default class BandwidthModule extends HttpModule {
    ...
    /**
     * @private
     * @member {Defer} BandwidthModule#_originalDelay
     */
    @enumerable(false)
    _originalDelay = null;
    ...
    _downloadSizes = [
        '350x350', '500x500', '750x750', '1000x1000', '1500x1500', '2000x2000',
        '2500x2500', '3000x3000', '3500x3500', '4000x4000'
    ]
    ...
    constructor(loadingType, settings = {})
    {
        ...
        this._extendDefaultSettings({
            data: {
                // 20 MB for upload, 4000x4000 image for download
                size: loadingType === 'upload' ? (20 * 1024 * 1024) : this._downloadSizes[9],
                multiplier: 2,
                maxRestarts: 5,
            }
        }).settings(settings);

        // override endpoint url to include image for download
        if (loadingType === 'download') {
            var baseUrl = this._settings.endpoint.split('/').slice(0, -1).join('/');
            this.settings({
                endpoint: baseUrl + '/random' + this._settings.data.size + '.jpg'
            });
        }

        this._originalDelay = this.settings().delay;
    ...
    start()
    {
        ...
        this._newRequest(type)._sendRequest(blob);
    ...
    _end()
    {
        ...
        var delay = settings.delay - (this._requestEndTime - this._requestStartTime);
        // A timeout or an abort occured, bypass the further requests and trigger the "end" event.
        if (this._intendedEnd || delay < 1000) {
            this._isRestarting = false;
            this.settings({
                delay: this._originalDelay,
            });
        ...
        // The request ended to early, restart it with an increased data size.
            ...
            if ((this._loadingType) === 'download') {
                var thisIndex = this._downloadSizes.indexOf(dataSettings.size);
                var nextIndex = thisIndex < this._downloadSizes.length ? thisIndex + 1 : thisIndex;
                size = this._downloadSizes[nextIndex];
            } else {
        ...
        console.log(`Original delay: ${this._originalDelay}`);
        console.log(`Current delay: ${settings.delay}`);
