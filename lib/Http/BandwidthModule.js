...
export default class BandwidthModule extends HttpModule {
    ...
    /**
     * Current number of restarts, test ends when this number reaches settings.data.maxRestarts
     * @private
     * @member {Defer} BandwidthModule#_restartCounter
     */
    @enumerable(false)
    _restartCounter = 0;
    ...
    _end()
    {
        ...
        if (this._intendedEnd || this._restartCounter === dataSettings.maxRestarts) {
        ...
        // The request ended to early, restart it with an increased data size.
            ...
            this._restartCounter += 1;
            var size;
            if ((this._loadingType) === 'download') {
                var thisIndex = this._downloadSizes.indexOf(dataSettings.size);
                var nextIndex = thisIndex < this._downloadSizes.length ? thisIndex + (dataSettings.multiplier - 1) : thisIndex;
